...
export async function sendPasswordResetEmail({ toEmail, userName, resetCode }) {
  ...
  // Set RESEND_FROM_EMAIL in Vercel env vars, e.g. "LifeAgent <noreply@yourdomain.com>"
  const resendFromEmail = process.env.RESEND_FROM_EMAIL;

  if (!resendApiKey || !resendFromEmail) {
    const missing = !resendApiKey ? 'RESEND_API_KEY' : 'RESEND_FROM_EMAIL';
    console.error(`[Email] Missing env var: ${missing}. Cannot send reset email.`);
    return { success: false, method: 'console', code: resetCode, error: `Missing env var: ${missing}` };
  }
  ...
  if (resendApiKey) {
    try {
      ...
      // Common issue: unverified sender domain. Guide the user.
      const hint = response.status === 403 || (errorMessage || '').toLowerCase().includes('domain')
        ? ' Hint: Verify your sender domain in Resend and set RESEND_FROM_EMAIL env var.'
        : '';
      return { success: false, method: 'console', code: resetCode, error: errorMessage + hint };
    } catch (err) {
