...
export async function ensureDbSchema() {
  ...
  await Promise.all(
    createQueries.map(sql => client.execute(sql).catch(e => console.warn('[ensureDbSchema] table create error:', e?.message)))
  );

  // 1b. Create indexes for performance optimization (eliminates unindexed table scans)
  const indexQueries = [
    `CREATE INDEX IF NOT EXISTS idx_habits_user ON habits(user_id)`,
    `CREATE INDEX IF NOT EXISTS idx_today_items_user_date ON today_items(user_id, date)`,
    `CREATE INDEX IF NOT EXISTS idx_transactions_user_date ON transactions(user_id, date)`,
    `CREATE INDEX IF NOT EXISTS idx_notes_user_date ON notes(user_id, is_trashed, date)`,
    ...
    `CREATE INDEX IF NOT EXISTS idx_reminders_user ON reminders(user_id, entity_type, entity_id)`,
    `CREATE INDEX IF NOT EXISTS idx_user_settings_user ON user_settings(user_id)`,
    `CREATE INDEX IF NOT EXISTS idx_chat_history_user ON chat_history(user_id)`,
    `CREATE INDEX IF NOT EXISTS idx_daily_metrics_user_email ON daily_metrics(user_email)`
  ];
  await Promise.all(
    indexQueries.map(sql => client.execute(sql).catch(() => {}))
  );

  // Create auto-cascade trigger so deleting a user directly in Turso Dashboard / CLI automatically deletes all child data
  // Note: daily_metrics has no user_id column - keyed by user_email only
  const cascadeTriggerSql = `
    ...
    BEGIN
      ...
      DELETE FROM daily_metrics WHERE user_email = OLD.email;
    END;
  `;
  await client.execute('DROP TRIGGER IF EXISTS cascade_delete_user_data').catch(() => {});
