  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const result = await db.execute({
        sql: 'INSERT INTO transactions (user_id, title, amount, type) VALUES (?, ?, ?, ?)',
        args: [userId, title, amount, type || 'spend']
      });
      return res.status(201).json({ id: Number(result.lastInsertRowid), title, amount, type });
    }
