...
public class WaterAlarmReceiver extends BroadcastReceiver {
  ...
  static void scheduleNext(Context c) {
    ...
    if (Build.VERSION.SDK_INT >= 23) a.setAndAllowWhileIdle(AlarmManager.RTC_WAKEUP,next.getTimeInMillis(),intent(c)); else a.set(AlarmManager.RTC_WAKEUP,next.getTimeInMillis(),intent(c));
  }
